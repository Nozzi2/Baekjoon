...
- 15:40 ~ 브레인 스토밍
    - 한계단씩 밟았을 떄의 최댓점수를 저장해서 마지막 계단까지 계산하고 최대값을 출력하면 될듯?
    - 근데 3개 연속으로 밟으면 안된다고 했으니 연속으로 밟았을 때의 최대값과 연속으로 밟지 않았을 떄의 최대값을 따로 저장해야할듯?
    - fo(n) 연속으로 밟았을 떄 최대값, fx(n) 연속으로 밟지 않았을 때 최대값
