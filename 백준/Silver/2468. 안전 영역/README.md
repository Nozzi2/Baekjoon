...
### 풀이 방법
- 00:45 ~ 브레인 스토밍
    - 일단 시간복잡도를 계산해봤을 때는 아래와 같다.
        - map의 최대 크기는 100x100이다.
        - 높이의 최대 높이는 100이므로 0~100까지 연산할 수 있다
        - 100x100  x 101 이렇게 계산하면 100만 정도 나오는데, 시간 제한이 1초이기 때문에 충분해보인다.
    - 100부터 0까지 높이를 점점 낮추면서 안전 영역의 갯수를 계산한다.
- 01:00 ~ 코드 작성
- 01:25 풀이 완료! (40분)
