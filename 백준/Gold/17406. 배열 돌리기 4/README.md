...
- 15:00 ~ 브레인 스토밍
    - 입력되는 값들은 항상 배열의 범위를 만족하기 때문에 level을 따로 계산할 필요는 없어보인다.
    - r, c, s 입력대로 수행하는 메소드를 만들어서 하면 될듯
    - 이전에 배열돌리기 1과 달리 이 문제는 배열을 연산마다 직접 수행해주고, 결과로 나온 배열에 최소값을 구하면 될 것 같다.
- 15:15 ~ 코드 작성
    - 코드 작성하다가 문득 한줄의 합에서 회전하는 만큼 생기고 빠진 배열의 칸을 더하고 빼면 되지 않을까? 생각했는데, 연산을 여러번 해야하기 때문에 이렇게 할 수 없다!
- 17:15 ~ 제출 후 반례 수정
    - 문제에서 요구하는 것은 입력된 순서대로 회전돌리기를 수행하는 것이 아니라, 회전돌리기가 여러개 주어졌을 때 순서를 조합해서 그중 최소값을 찾아내는 것이였다.
    - 조합써서 했는데, 배열이 제대로 전달되지 않아서 고생좀 했음…
- ~ 19:25 풀이 완료!
