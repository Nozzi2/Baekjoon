...
- 19:00 ~ 브레인 스토밍
    - 계속 낮은 값을 갖는 곳만을 DFS 탐색하면될듯?
- 19:05 ~ 코드 작성
    - 메모리 초과 뜸..
        - ArrayDeque쓰는 BFS로 바꿔볼까?
        →똑같이 메모리초과
    - 메모이제이션을 통해서 한번 갔던 길은 다시 가지 않도록 한다.
        - 시작점에서 dfs를 호출하여 반환값을 분기되는 경로의 도착하는 경우의 수를 반환하도록 한다.
        - 최종적으로는 시작점에서 모든 경우의 수가 더해지게 된다.
- 21:30 풀이 완료! (1시간 30분 (+식사 1시간))
